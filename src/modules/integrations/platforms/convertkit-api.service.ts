...
import { safeInt, safeFloat } from '../../../common/utils/safe-parse';

/**
 * ConvertKit (Kit) API service — email broadcast performance.
 *
 * API version: v3 (v4 Bearer auth rejects the API secret; v3 query-param auth works)
 * Docs: https://developers.kit.com/docs/api-v3
 *
 * Auth: `?api_secret={secret}` query param on every request.
 * Base URL: https://api.convertkit.com/v3
 *
 * Approach:
 *   1. GET /broadcasts?api_secret=... — list all broadcasts, filter by created_at in-memory.
 *   2. GET /broadcasts/{id}/stats?api_secret=... — per-broadcast aggregate stats.
 */
@Injectable()
export class ConvertKitApiService {
  private readonly logger = new Logger(ConvertKitApiService.name);
  private readonly BASE = 'https://api.convertkit.com/v3';

  private url(path: string, apiKey: string): string {
    return `${this.BASE}${path}?api_secret=${apiKey}`;
  }

  /**
   * @param apiKey     ConvertKit API secret (from CONVERTKIT_API_SECRET env var)
   * @param _accountId Not needed. Pass 'default'.
  ...
  async fetchCoreMetrics(
    ...
    for (const b of broadcasts.slice(0, 20)) {
      try {
        ...
        this.logger.warn(`ConvertKit: stats failed for broadcast ${b.id}: ${(err as Error).message}`);
      }
    }
  ...
  private async listBroadcasts(
    ...
    const resp = await fetchWithRetry(this.url('/broadcasts', apiKey));
    ...
    const body = await resp.json() as {
      broadcasts: Array<{ id: number; created_at: string }>;
    };
  ...
  private async getBroadcastStats(
    apiKey: string,
    broadcastId: number,
  ): Promise<{ recipients: number; open_rate: number | null; click_rate: number | null; unsubscribes: number }> {
    const resp = await fetchWithRetry(this.url(`/broadcasts/${broadcastId}/stats`, apiKey));

    if (!resp.ok) throw new BadRequestException(`ConvertKit broadcast stats failed (HTTP ${resp.status})`);

    const body = await resp.json() as {
      broadcast: { stats: { recipients?: number; open_rate?: number; click_rate?: number; unsubscribes?: number } };
    };
