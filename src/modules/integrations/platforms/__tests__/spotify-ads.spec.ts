...
import { SpotifyAdsApiService } from '../spotify-ads-api.service';
import fixture from '../__fixtures__/spotify-ads.fixture.json';

const dateRange = { from: '2024-01-15', to: '2024-01-21' };

describe('SpotifyAdsApiService', () => {
  let service: SpotifyAdsApiService;

  beforeEach(() => {
    service = new SpotifyAdsApiService();
    clearFetchMocks();
  });
  ...
  it('null fields — does not throw when metric fields are null/undefined', async () => {
    mockFetchResponse({
      data: [{ date: '2024-01-15', impressions: null, clicks: null, spend: null }],
    });
    await expect(
