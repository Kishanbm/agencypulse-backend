...
import { StackAdaptApiService } from '../stackadapt-api.service';

const dateRange = { from: '2024-01-15', to: '2024-01-21' };

describe('StackAdaptApiService', () => {
  let service: StackAdaptApiService;

  beforeEach(() => {
    service = new StackAdaptApiService();
    clearFetchMocks();
  });
          ...
          nodes: [
            {
              ...
              name: 'Test Campaign',
              stats: [
                { date: '2024-01-15', impressions: 74200, clicks: 1923, spend: 834.50, ctr: 0.0259, conversions: 67 },
                { date: '2024-01-16', impressions: 78400, clicks: 2042, spend: 882.10, ctr: 0.0260, conversions: 72 },
              ],
            },
          ],
        },
  ...
  it('empty data — returns [] when API returns empty results', async () => {
    mockFetchResponse({ data: { campaigns: { nodes: [] } } });
    const rows = await service.fetchCoreMetrics('test-token', 'test-account', dateRange);
    expect(rows).toEqual([]);
  });
  ...
  it('null fields — does not throw when metric fields are null/undefined', async () => {
    mockFetchResponse({
      data: {
        campaigns: {
          nodes: [
            { stats: [{ date: '2024-01-15', impressions: null, clicks: null, spend: null }] },
          ],
        },
      },
    });
